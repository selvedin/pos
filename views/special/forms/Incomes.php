...
<div class="added-items">
	...
	<tbody>
		...
		foreach ($items as $item) {
		    ...
		    $total+=$item->price;
		    echo "<tr><td><input type='text' value='$item->id_item' name='incomes[items][$item->id_product][id_item]'/></td><td><input type='text' value='$item->id_product' name='incomes[items][$item->id_product][id_product]'/></td><td>$product->name</td><td>$unit->short</td><td>$product->price</td><td><input type='text' value='$item->qnt' name='incomes[items][$item->id_product][qnt]' /></td><td><input type='text' value='$item->price' name='incomes[items][$item->id_product][total]' /></td><td><input type='text' value='0' name='incomes[items][$item->id_product][discount]' /></td><td><input type='text' class='final-price' value='$item->price' name='incomes[items][$item->id_product][final]' /></td></tr>";
		}
	    }
	    ?>
...
<script>
	...
	$("div.tab-content div#tab1").append($("form#incomes-form"));
	...
	$(document).on("keydown", "table.pos-result tr td input", function(e){
		...
		$("table.added-items tbody").append("<tr><td><input type='text' value='0' name='incomes[items]["+id+"][id_item]'/></td><td>"+name+"</td><td>"+unit+"</td><td>"+price+"</td><td><input type='text' value='"+qnt+"' name='incomes[items]["+id+"][qnt]' /></td><td><input type='text' value='"+ total.toFixed(2) +"' name='incomes[items]["+id+"][total]' /></td><td><input type='text' value='"+ disc +"' name='incomes[items]["+id+"][discount]' /></td><td><input type='text' class='final-price' value='"+final.toFixed(2)+"' name='incomes[items]["+id+"][final]' /></td></tr>");
		$("table.added-items").find("input").prop('disabled', true);
		$("table.pos-result tr").remove();
	...
	$("#incomes-invoice_amount").val(finalPrice.toFixed(2));
    }
</script>
