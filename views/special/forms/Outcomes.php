...
<div class="added-items">
	...
	<tbody>
		...
		$items = \app\models\Outcomeitems::find()->where("id_income=$model->id_income")->all();
		foreach ($items as $item) {
		    $product = \app\models\Products::findOne($item->id_product);
		    $unit = \app\models\Units::findOne($product->id_unit);
		    $total+=$item->price;
		    echo "<tr><td><input type='text' value='$item->id_item' name='outcomes[items][$item->id_product][id_item]'/></td><td><input type='text' value='$item->id_product' name='outcomes[items][$item->id_product][id_product]'/></td><td>$product->name</td><td>$unit->short</td><td>$product->price</td><td><input type='text' value='$item->qnt' name='outcomes[items][$item->id_product][qnt]' /></td><td><input type='text' value='$item->price' name='outcomes[items][$item->id_product][total]' /></td><td><input type='text' value='0' name='outcomes[items][$item->id_product][discount]' /></td><td><input type='text' class='final-price' value='$item->price' name='outcomes[items][$item->id_product][final]' /></td></tr>";
		}
	    }
	    ?>
...
<script>
	...
	$("div.tab-content div#tab1").append($("form#outcomes-form"));
	...
	$(document).on("keydown", "table.pos-result tr td input", function(e){
		...
		$("table.added-items tbody").append("<tr><td><input type='text' value='0' name='outcomes[items]["+id+"][id_item]'/></td><td>"+name+"</td><td>"+unit+"</td><td>"+price+"</td><td><input type='text' value='"+qnt+"' name='outcomes[items]["+id+"][qnt]' /></td><td><input type='text' value='"+ total.toFixed(2) +"' name='outcomes[items]["+id+"][total]' /></td><td><input type='text' value='"+ disc +"' name='outcomes[items]["+id+"][discount]' /></td><td><input type='text' class='final-price' value='"+final.toFixed(2)+"' name='outcomes[items]["+id+"][final]' /></td></tr>");
		$("table.added-items").find("input").prop('disabled', true);
		$("table.pos-result tr").remove();
	...
	//$("#outcomes-invoice_amount").val(finalPrice.toFixed(2));
    }
</script>
